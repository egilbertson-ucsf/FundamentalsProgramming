...
def grade_within_cluster(labels):
    if 'GarGene' in labels:
        labels.remove('GarGene')
...
def grade(labels, clusters):
    ...
    for label, cluster in zip(labels, clusters):
        if cluster in labels_split_by_clusters:
            labels_split_by_clusters[cluster].append(label)
        else:
            labels_split_by_clusters[cluster] = [label]

    total_distance = 0
    max_distance = 0
    for cluster in labels_split_by_clusters:
        dist, max = grade_within_cluster(labels_split_by_clusters[cluster])
        total_distance += dist
        max_distance += max
    return adj_rand, (1 - (total_distance / max_distance)) * 100
